            ...
            selection.each(function (data) {
                ...
                var arcs = container.selectAll("g.slice")     //this selects all <g> elements with class slice (there aren't any yet)
                    ...
                    .on("mouseout", function (d, i) {
                        d3.select(this).select("path").transition()
                            .duration(500)
                            .attr("fill", (color(i) === '#1f77b4') ? '#b5cf6b' : color(i))
                            .attr("d", arc);
                        d3.select(this).select("text").transition().duration(500)
                            .attr("dy", ".35em")
                            .attr("text-anchor", "middle")
                ...
                function newGraph(d) {
                    ...
                    if(d.data.data.length){
                        ...
                        console.log(graph);
                        var pieChart = graph.pieGraph().title(d.data.data[0].children[0].name + " in "+ d.data.data[0].os + " by nodes").previousData(data).width(width);
                        //console.log(pieChart);
                        d3.select(that).datum(tempData).call(pieChart);
                    }
                }
                function previousGraph() {
                    var pieChart = graph.pieGraph().title(title).width(width);
                ...
                arcs.append("svg:path")
                    .attr("fill", function (d, i) { return (color(i) === '#1f77b4') ? '#b5cf6b' : color(i); }) //set the color for each slice to be chosen from the color function defined above
